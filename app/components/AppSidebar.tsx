...
export default function AppSidebar() {
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-4 left-1/2 z-50 flex w-[92%] -translate-x-1/2 items-center justify-around gap-2 rounded-full border border-white/40 bg-white/70 px-3 py-2 text-[#111111] shadow-[0_12px_30px_rgba(0,0,0,0.15)] backdrop-blur-md lg:hidden">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive =
            pathname === item.href ||
          ...
          return (
            <Link
              key={item.label}
              href={item.href}
              className={`flex flex-1 flex-col items-center gap-1 rounded-full px-2 py-2 text-[11px] font-semibold transition-all duration-300 ease-out ${
                isActive
                  ? "bg-white/70 text-[#111111] shadow-[0_8px_20px_rgba(0,0,0,0.18)] backdrop-blur"
                  : "text-[#111111]/60"
              }`}

...
const SERVICE_NAME = "podium-web";
const APP_ENVIRONMENT = process.env.VERCEL_ENV ?? process.env.NODE_ENV ?? "local";
...
function isTelemetryEnabled() {
  return Boolean(process.env.OPSCOMPANION_API_KEY);
}

function toAttributeValue(value: AttributeValue): OtlpAnyValue {
  if (Array.isArray(value)) {
    return { arrayValue: { values: value.map((item) => toAttributeValue(item)) } };
  }
...
export function trackLog({ eventName, body, severity = "INFO", attributes }: LogInput) {
  return postOtlp("logs", {
    resourceLogs: [
      {
        resource: { attributes: normalizeAttributes(attributes) },
        scopeLogs: [
          {
